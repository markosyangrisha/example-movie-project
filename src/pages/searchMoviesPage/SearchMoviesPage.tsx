...
const SearchMoviesPage: FC = () => {
	...
	return (
		<div className='search-movies__block'>
			...
			{ data?.results.length === 0 ? <p>Not found movies</p> : data?.results.map(movie => (
				<div key={movie.id} className='search-movie__card'>
					<img
					...
					<div className='search-movie__card-info'>
						<span className='search-card-info__date'>{movie.release_date}</span>
						<span className='search-movie__card-title'>{movie.title}</span>
					</div>
					<Icons.Favorite className='search-movie__card-favorite' />
